...
if __name__=="__main__":
    ...
    simset = ["FID_FIXED48"+x for x in ["_NOB","","_HIB"]]
    setname = "varyingB"
    times = np.array([0.035]) # np.array([0.9]) # [0.9] # 3.5 Myr = tstarformed + 0.2 Myr
    times = np.atleast_1d(times)
    zoom = 0.5
    setname = setname+str(times[-1])+"Myr_"+"zoom"+str(zoom)+"_"
    setname = setname.replace(".","p") # the extra dot confuses latex
    #timesL = [str(x)+r' Myr' for x in times]
    timesL = ['' for x in times]
    timesin = [(time,"code") for time in times]
    for los in "xyz":
        figname = setname+"_"+los
        # NH Maps
        for hydro in ["maxBmag","NH"]:
            contours = []#["Ionised"]
            MakeFigure(simset,timesin,name=figname,
                       los=los,hydro=hydro,Slice=False,wsink=False,
                       timeL=timesL,zoom=zoom,forcerun=True,
                       contours=contours,
                       plotcolorbar=True)
'''

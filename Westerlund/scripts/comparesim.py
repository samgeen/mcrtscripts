...
def findshellradius(snap):
    ...
    nx = hydro.ncells
    radius = 0.0
    mask = np.where(hydro.T[0:nx] > 5e3)[0]
    if len(mask) > 0:
...
def plotprofiles(setname,snap):
    ...
    plt.savefig("../plots/oned/"+setname+"/"+simname+"/1Dhydrotest_"+str(snap.OutputNumber())+".pdf")
...
if __name__=="__main__":
    ...
    simsets = {"fbset":fbset,"windonly_cdmaskset":windonlyset,"densityset":densityset}

...
def run():
    ...
    # Calculated from mass inside isothermal distribution = 2e5 Msun (SFE = 50%)
    n0 = 240000.0 # cm^-3
    T0 = 10.0 # K
    integrator.Setup(ncells = ncells, # Number of cells in the grid
            rmax = 100.0*wunits.pc, # maximum radius of simulation volume
            n0 = n0, # atoms / cm^-3
            T0 = T0, # K
    ...
    # Now let's add a stellar cluster. 
    ...
    radiation = True
    wind = True
    cluster = clustersource.ClusterSource(mcluster,0.014,radiation=radiation,wind=wind,rotating=True)
    weltgeist.sources.Sources().AddSource(cluster)
    ...
    if not weltgeist.cooling.cooling_on:
        extra += "_nocooling"
    folder = "../outputs/"+str(mcluster)+"Msun_n"+str(int(n0))+"_w2_N"+str(ncells)+"_"+fbname+"_coolingfix2"+extra+"/"

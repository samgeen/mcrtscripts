...
class ClusterPrecomputed(object):
    ...
    def __init__(self,mass,metal,seed=1,maxtime_Myr = 130.0,dt_Myr=0.01,rotating=True):
        # NOTE: mass can be one mass or a list
        # if mass is one value, populate a cluster with these masses
        # if mass is multiple values, use this directly as a cluster
        self._mass = mass
        ...
        self._stars = [Star(mass,metal,rotating) for mass in self._masses]
        if len(self._stars) > 0:
        ...
        self._windml = np.zeros(len(self._times))
        ...
        self._lionising = np.zeros(len(self._times))
        self._lnonionising = np.zeros(len(self._times))
        for star in self._stars:
            self._windml += star.WindMassLoss(self._times)
            ...
            self._lionising += star.LIonising(self._times)
            self._lnonionising += star.LNonIonising(self._times)
        self._windmlFunc = None
        self._windlumFunc = None
        self._windmomFunc = None
        self._TeffFunc = None
    ...
    def WindMassLoss(self, t):
        if self._windmlFunc is None:
            self._windmlFunc = scipy.interpolate.interp1d(self._times,self._windml,fill_value="extrapolate")
        return self._windmlFunc(t)
    ...
    def Teff(self,t):
        if self._TeffFunc is None:
            self._TeffFunc = scipy.interpolate.interp1d(self._times,self._Teff,fill_value="extrapolate")
        return self._TeffFunc(t)
    ...
    def LIonising(self, t):
        if self._lionisingFunc is None:
            self._lionisingFunc = scipy.interpolate.interp1d(self._times,self._lionising,fill_value="extrapolate")
        return self._lionisingFunc(t)

    def LNonIonising(self, t):
        if self._lnonionisingFunc is None:
            self._lnonionisingFunc = scipy.interpolate.interp1d(self._times,self._lnonionising,fill_value="extrapolate")
        return self._lnonionisingFunc(t)

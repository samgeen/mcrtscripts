...
def windradiusratio_analytic(simname):
    ...
    sim = hamusims[simname]
    metal = 0.014
    masstxt = simname.split("_")[-1]
    if masstxt == "DENSE":
        masstxt = simname.split("_")[-2]
    starmass = float(masstxt)
    #starmass = float(str.join("",[i for i in simname if i.isdigit()])) # Breaks if other digits in the name
    tcreated, sfe = starrelations.runforsim(simname,"firsttime")  
...
if __name__=="__main__":
    ...
    allwindCRnames = ["UVWINDCR25_"+str(num) for num in [120]]
    allwindCRnames += ["UVWINDCR26_"+str(num) for num in [120]]
    allfbnames = alluvnames+allwindnames+allwindCRnames
    allnames = ["NOFB"]+allfbnames
    for func in [windradiusratio]:
        run(func,(allwindnames+allwindCRnames,["UVWIND_120_DENSE"]),

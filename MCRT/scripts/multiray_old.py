...
def PlotForSims(simnames, times,hydro="rho", name = ""):
    ...
    # Add text - Times
    tax = fig.add_axes([0.05, 0.1, 0.03, 0.8])
    for itime in range(0,ntimes):
        timestr = str(times[itime])+" Myr"
        tax.text(0.1,0.1+(ntimes-itime-0.5)/ntimes,timestr,rotation="vertical")
    tax.set_axis_off()
    # Add text - Simnames
    sax = fig.add_axes([0.0, 0.0, 0.8, 0.03])
    for isim in range(0,nsim):
        simstr = sims[isim].Label()
        sax.text((isim+0.5)/nsim,0.0,
                 simstr)
    sax.set_axis_off()
    # Save figure
    plt.tight_layout()
    ...
    if len(name) > 0:
        suffix = "_"+name
    figname = "../plots/vis/multiray/multiray_"+hydro+suffix+".eps"
...
if __name__=="__main__":
    ...
    times = [1.25,1.25*2.0,1.25*3.0,1.25*4.0]
    PlotForSims(simnames,times,name="compact")
    simnames = ["N00_M4_B02","N48_M4_B02","N48_M4_B00"]
    times = [1.25,1.25+2.0]
    PlotForSims(simnames,times,name="fiducial")
    simnames = ["N00_M4_B02","N47_M4_B02","N48_M4_B02","N49_M4_B02"]
    times = [1.25,1.25+1.0,1.25+2.0,1.25+3.0]
    PlotForSims(simnames,times,name="fluxes")

...
weltgeistpath = "C:\\Users\\User\\OneDrive\\Programming\\Astro\\Weltgeist"
sys.path.append(weltgeistpath)
...
def SoundSpeed(temperature):
    ci = np.sqrt(2.0 * gamma * kB * temperature * X / mH)
    return ci

def ShellDensity(n0,T0,vs,)
...
def run_example():
    ...
    integrator.Setup(ncells = ncells, # 256 cells
            ...
            n0 = n0, # atoms / cm^-3
            T0 = T0, # K
            gamma = gamma) # monatomic gas (close enough...)

...
def plotallprofiles(simnames,hydro,centre,labels=None,xlims=None,powfile=None,suffix=None,plotmean=False,xscale="log"):
    '''
    ...
    for simname in simnames:
        ...
        for currsnap in snaps: 
            ...
            if "starpos" in suffix:
                ...
                centre = starpos
                snap = currsnap
        r,p = rayprof.makeprofsHamu(snap,hydro,centre)
        radii = r[0:nray]
        ...
        # Make median line
        if not plotmean:
            plt.plot(radii,percs[0:nprofs,25],"k",label=label,color=colour)
        # Plot the mean line 
        else:
            ...
            rsph = rsph[mask]
            psph = psph[mask]
            plt.plot(rsph,psph,"k--",label=label,color=colour)
...
if __name__ == "__main__":
    ...
    time = None
    starpos = None
    plotallprofiles(simnames,"nH",starpos,labels,
                    xlims=[0.03,25.0],powfile=None,suffix="starpos_nofb",plotmean=False,xscale="log")
    plotallprofiles(simnames,"Pram",starpos,labels,
                    xlims=[0.03,25.0],powfile=None,suffix="starpos_nofb",plotmean=False,xscale="log")

...
def run(func,simnames,linelabels=[],timerange=None,nsnsub=False,
    ...
    if showphotonlegend:
        plotlines, labels = linestyles.irlegend()
...
if __name__=="__main__":
    simnames = ["NIR-NUV","IRP-UVP","NIR-UVNP"]
    run(meaniondensinsnap,simnames)
    run(meanionpressinsnap,simnames)
    run(momentuminsnap,simnames)
    run(totalmomentuminsnap,simnames)

...
class ProfAna(object):
    def __init__(self,sim,collapsemodel=False):
        ...
        self._collapsemodel = collapsemodel
        r0 *= outflowmodel.pcincm
        self._r0 = r0
        self._r0w = r0**w
        self._minusw = -w

    def rho(self,r,t=0.0):
        n0 = self._n0
        if self._collapsemodel:
            tff = findtff(self)
            n0 /= (1.0-t/tff)**2
            print n0
        if r <= self._r0:
            return n0
        return n0 * r**self._minusw * self._r0w
    ...
    def vesc(self,r,t=0.0):
        ...
        if self._collapsemodel:
            tff = findtff(self)
            n0 /= (1.0-t/tff)**2
        # With two-phase model we need to find the mass inside R
        M = 0.0
        if r <= self._r0:
            M = 4.0/3.0*np.pi*(r**3)*self._n0*mp
        else:
            Mc = 4.0/3.0*np.pi*(self._r0)*n0*mp
            Mpow0 = 4.0*np.pi*self._r0w*(self._r0**(3.0+self._minusw))* \
                n0*mp/(3.0+self._minusw)
            Mpow  = 4.0*np.pi*self._r0w*(       r**(3.0+self._minusw))* \
                n0*mp/(3.0+self._minusw)
            M = Mpow - Mpow0 + Mc # Correct for flat centre
        #vfact = 2.0 # Escape velocity
        vfact = 2.0 * 3.0/5.0 # Virial velocity
        vesc = np.sqrt(vfact * G * M / r)
        vesc /= 1e5 # expects km/s
        return -vesc # acting in opposite to radial direction
...
def ComputeExpansion(sim,solntype="full",proftype="sim",collapsemodel=False):
    '''
    ...
    if proftype == "ana":
        ...
        profana = ProfAna(sim,collapsemodel)
    ...
    # Set function to use
    rcloud = 3.6*outflowmodel.pcincm # Not really needed now
    if solntype == "full":

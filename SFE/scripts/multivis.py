...
def plotforsnap(snap, ax, hydro,dolengthscale=False,doxHII=True,imtype = "projection"):
    ...
    def makeray(snap,hydro,mapsize=512,rhoweighting=True):
        cam  = Camera(center=centre, line_of_sight_axis='z', 
                      region_size=size, up_vector='y', 
                      map_max_size=mapsize, log_sensitive=True)
        hydro_op = pymses_func(snap,hydro,"projection",rhoweighting=rhoweighting)
    ...
    def makeslice(snap,hydro,mapsize=512,rhoweighting=True):
        cam  = Camera(center=centre, line_of_sight_axis='z', 
                      region_size=size, up_vector='y', 
                      map_max_size=mapsize, log_sensitive=True)
    ...
    for hydro in hydros:
        # Get data
        if hydro != "RGB":
            slc = makeimageHamu(snap,hydro,mapsize=mapsize)
            if not "xH" in hydro:
                slc = np.log10(slc)
        else:
